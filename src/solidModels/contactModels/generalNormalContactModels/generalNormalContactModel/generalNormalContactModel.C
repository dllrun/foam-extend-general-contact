...
defineTypeNameAndDebug(generalNormalContactModel, 0);
defineRunTimeSelectionTable(generalNormalContactModel, dictionary);


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

generalNormalContactModel::generalNormalContactModel
(
    const word& name,
    const fvPatch& patch,
    ...
    )
{
    if (slavePatchID == -1)
    {
        FatalErrorIn("generalNormalContactModel::generalNormalContactModel")
            << "slavePatchID cannot be -1" << abort(FatalError);
    }
}


generalNormalContactModel::generalNormalContactModel(const generalNormalContactModel& nm)
:
    name_(nm.name_),
    patch_(nm.patch_),
