...
#include "generalStandardPenalty.H"
#include "volFields.H"
#include "fvc.H"
...
// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //
  ...
  defineTypeNameAndDebug(generalStandardPenalty, 0);
  addToRunTimeSelectionTable(generalNormalContactModel, generalStandardPenalty, dictionary);


// * * * * * * * * * * * * Private Member Functions * * * * * * * * * * * * * //


void generalStandardPenalty::calcPenaltyFactor() const
{
    if (penaltyFactor_ > -SMALL)
    {
        FatalErrorIn("void generalStandardPenalty::calcPenaltyFactor() const")
...
generalStandardPenalty::generalStandardPenalty
(
    const word& name,
    const fvPatch& patch,
...
void generalStandardPenalty::correct
(
...
scalar generalStandardPenalty::penaltyFactor() const
{
    if (penaltyFactor_ < -SMALL)
    {
        calcPenaltyFactor();
...
scalar generalStandardPenalty::updatePenaltyScale(const scalar previousPenaltyScale)
{
...
void generalStandardPenalty::autoMap(const fvPatchFieldMapper& m)
{
    if (debug)
    {
        InfoIn
    ...
    generalNormalContactModel::autoMap(m);
...
void generalStandardPenalty::writeDict(Ostream& os) const
{
